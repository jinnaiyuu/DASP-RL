...
SarsaLearner::SarsaLearner(ALEInterface& ale, Features *features,
		Parameters *param, int seed) :
		RLLearner(ale, param, seed) {
	...
	// YJ. This implementation does not use optimistic initial value.
	// I can try set high values to see how it works.
	...
	if (toSaveCheckPoint) {
		...
		saveThreshold = (totalNumberFrames / saveWeightsEveryXFrames)
				* saveWeightsEveryXFrames;
		ofstream learningConditionFile;
		nameForLearningCondition = checkPointName + "-learningCondition-Frames"
				+ to_string(saveThreshold) + "-finished.txt";
		string previousNameForLearningCondition = checkPointName
				+ "-learningCondition.txt";
		rename(previousNameForLearningCondition.c_str(),
				nameForLearningCondition.c_str());
...
void SarsaLearner::updateQValues(vector<long long> &Features,
...
void SarsaLearner::saveCheckPoint(int episode, int totalNumberFrames,
		vector<float>& episodeResults, int& frequency,
		vector<int>& episodeFrames, vector<double>& episodeFps) {
	ofstream learningConditionFile;
	string newNameForLearningCondition = checkPointName
			+ "-learningCondition-Frames" + to_string(saveThreshold)
			+ "-writing.txt";
	int renameReturnCode = rename(nameForLearningCondition.c_str(),
			newNameForLearningCondition.c_str());
	if (renameReturnCode == 0) {
		nameForLearningCondition = newNameForLearningCondition;
		learningConditionFile.open(nameForLearningCondition.c_str(),
				ios_base::app);
		int numEpisode = episodeResults.size();
		for (int index = 0; index < numEpisode; index++) {
			learningConditionFile << "Episode "
					<< episode - numEpisode + 1 + index << ": "
					<< episodeResults[index] << " points,  "
					<< episodeFrames[index] << " frames,  " << episodeFps[index]
					<< " fps." << endl;
		}
		...
		episodeFps.clear();
		learningConditionFile.close();
		newNameForLearningCondition.replace(
				newNameForLearningCondition.end() - 11,
				newNameForLearningCondition.end() - 4, "finished");
		rename(nameForLearningCondition.c_str(),
				newNameForLearningCondition.c_str());
		nameForLearningCondition = newNameForLearningCondition;
	}

	//write parameters checkPoint
	string currentCheckPointName = checkPointName + "-checkPoint-Frames"
			+ to_string(saveThreshold) + "-writing.txt";
	...
	for (unsigned long long groupIndex = 0; groupIndex < numGroups;
			++groupIndex) {
	...
	for (auto it = featureTranslate.begin(); it != featureTranslate.end();
			++it) {
	...
	string previousVersionCheckPoint = checkPointName + "-checkPoint-Frames"
			+ to_string(saveThreshold - saveWeightsEveryXFrames)
			+ "-finished.txt";
	...
	currentCheckPointName.replace(currentCheckPointName.end() - 11,
			currentCheckPointName.end() - 4, "finished");
...
void SarsaLearner::loadCheckPoint(ifstream& checkPointToLoad) {
	...
	while (checkPointToLoad >> featureIndex
			&& checkPointToLoad >> featureToGroup) {
	...
	for (int episode = episodePassed + 1;
		...
		while (!ale.game_over()) {
			...
			if (!ale.game_over()) {
				...
				Fnext.clear();
				features->getActiveFeaturesIndices(ale.getScreen(),
						ale.getRAM(), Fnext);
		...
		elapsedTime = double(tvDiff.tv_sec)
				+ double(tvDiff.tv_usec) / 1000000.0;

		double fps = double(ale.getEpisodeFrameNumber()) / elapsedTime;
		int ram = getRAMConsumption();
		printf(
				"episode: %d,\t%.0f points,\tavg. return: %.1f,\t%d frames,\t%.0f fps,\t%d KBram\n",
				episode, cumReward - prevCumReward,
				(double) cumReward / (episode), ale.getEpisodeFrameNumber(),
				fps, ram);
...
void SarsaLearner::evaluatePolicy(ALEInterface& ale, Features *features) {
	...
	for (int episode = 1; episode < numEpisodesEval; episode++) {
		...
		resultFile << "Episode " << episode << ": " << cumReward - prevCumReward
				<< std::endl;
		printf(
				"episode: %d,\t%.0f points,\tavg. return: %.1f,\t%d frames,\t%.0f fps\n",
				episode, (cumReward - prevCumReward),
				(double) cumReward / (episode), ale.getEpisodeFrameNumber(),
				fps);
		features->clearCash();
	...
	resultFile << "Average: " << (double) cumReward / numEpisodesEval
			<< std::endl;
	resultFile.close();
	...
	for (unsigned long long index = 0; index < activeGroupIndices.size();
		...
		if (groups[groupIndex].features.size() != groups[groupIndex].numFeatures
			...
			for (unsigned long long i = 0;
					i < groups[groupIndex].features.size(); ++i) {
			...
			groups[groupIndex].numFeatures = groups[groupIndex].numFeatures
					- groups[groupIndex].features.size();
