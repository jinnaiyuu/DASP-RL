...
void Parameters::printHelp(char** argv) {
	...
	ANSI_COLOR_RED, ANSI_COLOR_RESET);
	printf(
			"   -r     %s[REQUIRED]%s path to the rom to be played by the agent.\n",
			ANSI_COLOR_RED, ANSI_COLOR_RESET);
	printf(
			"   -t     %s[REQUIRED IF SAVE_TRAJECTORY = 1]%s path to file that will store the agent's trajectory.\n",
			ANSI_COLOR_RED, ANSI_COLOR_RESET);
	printf(
			"   -w     If one wants to save intermediate weights, this is prefix to files that will store the agent's learned weights every FREQUENCY_SAVING episodes.\n");
	printf(
			"   -l     If one wants to load an stored set of weights, this should contain the path to such file.\n");
	printf(
			"    -n    If oen wants to save temporary intermediate weights locally, and then let the server to grap check points from local hosts, then this is required. Please provide the name of the job.\n");
...
Parameters::Parameters(int argc, char** argv) {
	...
	if (this->toSaveTrajectory
			&& this->getSaveTrajectoryPath().compare("") == 0) {
		printHelp(argv);
		exit(1);
	}
}
...
void Parameters::readParameters(int argc, char* argv[]) {
	...
	if (this->getRomPath().compare("") == 0
			|| this->getConfigPath().compare("") == 0 || this->getSeed() == 0) {
		printHelp(argv);
		exit(-1);
	}
}
...
void Parameters::parseParametersFromConfigFile(std::string cfgFileName) {
	...
	if (cfgFile.is_open()) {
		...
		printf(
				"Unable to open the file '%s', defined as the configuration file.\n",
				cfgFileName.c_str());
	}
	...
	this->setNumStepsPerAction(
			atoi(parameters["NUM_STEPS_PER_ACTION"].c_str()));
	...
	this->setSubtractBackground(
			atoi(parameters["SUBTRACT_BACKGROUND"].c_str()));
	this->setToSaveTrajectory(atoi(parameters["SAVE_TRAJECTORY"].c_str()));
	this->setOptimisticInitialization(
			atoi(parameters["OPTIMISTIC_INIT"].c_str()));

	this->setFrequencySavingWeights(
			atoi(parameters["FREQUENCY_SAVING"].c_str()));
	...
	if (parameters.count("FINAL_EXPLORATION_FRAME") > 0) {
		this->setFinalExplorationFrame(
				atoi(parameters["FINAL_EXPLORATION_FRAME"].c_str()));
	} else {
	...
	if (parameters.count("FINAL_NUMBER_OF_BLOBS") > 0) {
		this->setFinalNumberOfBlobs(
				atoi(parameters["FINAL_NUMBER_OF_BLOBS"].c_str()));
	} else {
...
void Parameters::setStepsPerPlanning(int a) {
	steps_per_planning = a;
}
void Parameters::setDaspSequenceLength(int a) {
	dasp_sequence_length = a;
}
void Parameters::setSearchMethod(std::string a) {
	search_method = a;
}
int Parameters::getPlanningEpisodes() {
	return planning_episodes;
}
int Parameters::getStepsPerPlanning() {
	return steps_per_planning;
}
int Parameters::getDaspSequenceLength() {
	return dasp_sequence_length;
}
