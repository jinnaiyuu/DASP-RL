...
void IW1Search::print_path(TreeNode * node, int a) {
	std::cerr << "Path, return " << node->v_children[a]->branch_return
			<< std::endl;

	while (!node->is_leaf()) {
		TreeNode * child = node->v_children[a];

		std::cerr << "\tAction " << a << " Reward " << child->node_reward
				<< " Return " << child->branch_return << " Terminal "
				<< child->is_terminal << std::endl;
...
int IW1Search::expand_node(TreeNode* curr_node, std::queue<TreeNode*>& q) {
	...
	std::vector<bool> isUsefulAction(PLAYER_A_MAX, true);
	if (action_sequence_detection) {
		if (!trajectory.empty()) {
			std::vector<Action> p = getPreviousActions(curr_node,
...
//			}
	...
	for (int a = 0; a < num_actions; a++) {
		...
		if (leaf_node) {
			...
			m_env->setState(child->state);
			m_env->getRAM();

			if (check_novelty_1(getRAM(child->state))) {
				update_novelty_table(getRAM(child->state));
...
void IW1Search::expand_tree(TreeNode* start_node) {
	...
	update_novelty_table(getRAM(start_node->state));
	int num_simulated_steps = 0;
