...
PlayerAgent::PlayerAgent(OSystem* _osystem, RomSettings* _settings) :
	...
	std::string rom_file = settings.getString("rom_file", false);
	size_t pos = 0;
	...
	if (extended_action_set <= 0) {
		...
		// 4 buttons (o, x,...)
		// 4 LR buttons
		std::vector<Action> playstation_actions = available_actions;
		for (int i = 0; i < 3; ++i) {
		...
		std::vector<Action> simple_set(available_actions.begin(), available_actions.begin() + 6);
...
void PlayerAgent::record_action(Action a) {
	if (episode_number == 0) {
		...
		state_trajectory.push_back(
				new ALEState(*curr_state, curr_state->serialize()));
	}
}
...
void PlayerAgent::end_game() {
	...
	if (record_trajectory && episode_number == 1) {
		...
		for (size_t i = 0; i < state_trajectory.size(); i++) {
			output << state_trajectory[i]->serialize() << "<endstate>";
		...
		std::cout << "\n";
		std::cout << "Action Trajectory ";
		for (size_t i = 0; i < action_trajectory.size(); i++) {
			std::cout << action_trajectory[i] << " ";
		}
		std::cout << "\n";
	}
